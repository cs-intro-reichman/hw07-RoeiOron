...
public class Koch {
	...
	double xTip = (Math.sqrt(3) / 6.0) * (y1 - y2) + 0.5 * (x1 + x2);
    double yTip = (Math.sqrt(3) / 6.0) * (x2 - x1) + 0.5 * (y1 + y2);
		
	curve(n - 1, x1, y1, xA, yA);       
    curve(n - 1, xA, yA, xTip, yTip);
    curve(n - 1, xTip, yTip, xB, yB);   
	...
	public static void snowFlake(int n) {
        ...
        double x3 = 0.5, y3 = 0.99;

        curve(n, x1, y1, x3, y3);   // מצייר את הצלע הראשונה
		curve(n, x3, y3, x2, y2);   // מצייר את הצלע השנייה
		curve(n, x2, y2, x1, y1);
	}
}
